document.addEventListener("DOMContentLoaded", () => {
  ...
  // 1. LÓGICA GLOBAL DE AUTENTICAÇÃO E UI
  if (
    !usuarioLogado &&
    !["index.html", "cadastro.html", "cadastro_empresa.html"].includes(
      currentPage
    )
  ) {
  ...
  // 2. LÓGICA DA BARRA DE PESQUISA (GLOBAL)
  ...
  if (currentPage === "index.html") setupLoginPage();
  else if (
  ...
  // 4. FUNÇÕES DE SETUP DAS PÁGINAS
    ...
    const carregarPerfil = async () => {
      try {
        ...
        document.getElementById(
          ...
          aluno.curso
        } - ${aluno.universidade}</p>${botaoChat}`;
        ...
        if (projetos.length > 0)
          ...
          listaProjetosDiv.innerHTML =
            '<p class="texto-secundario">Este usuário ainda não publicou nenhum projeto.</p>';
      } catch (err) {
  ...
  function setupChatPage() {
    ...
    const carregarMensagens = async (conversaId, nome, foto) => {
      ...
      try {
        ...
        mensagens.forEach((m) => {
          const classe =
            m.remetente_id == usuarioLogado.id ? "enviada" : "recebida";
  ...
  // 5. EVENT LISTENERS GLOBAIS PARA AÇÕES DINÂMICAS
...
// 6. FUNÇÃO UTILITÁRIA PARA CRIAR HTML DE POST
