...
// --- Criação das Tabelas (Estrutura Final) ---
db.serialize(() => {
  db.run(
    `CREATE TABLE IF NOT EXISTS alunos (id INTEGER PRIMARY KEY, nome TEXT NOT NULL, email TEXT NOT NULL UNIQUE, senha TEXT NOT NULL, curso TEXT, universidade TEXT, foto_perfil_path TEXT)`
  );
  db.run(
...
// --- Configurações do Express e Multer ---
const app = express();
...
app.get("/api/pesquisa/alunos", (req, res) => {
  ...
  const sql = `SELECT id, nome, foto_perfil_path, curso FROM alunos WHERE nome LIKE ? LIMIT 10`;
  db.all(sql, [`%${termo}%`], (err, alunos) => {
    if (err) {
...
app.post("/api/login/empresa", (req, res) => {
  ...
  db.get(sql, [email], (err, empresa) => {
    ...
    const { senha, ...dadosEmpresa } = empresa;
    res.status(200).json({
      sucesso: true,
...
app.post("/api/cadastro/aluno", upload.single("fotoPerfil"), (req, res) => {
  const { nome, email, curso, universidade, senha } = req.body;
  const fotoPath = req.file ? `/uploads/${req.file.filename}` : null;
  const senhaHash = md5Hash(senha);
  const sql = `INSERT INTO alunos (nome, email, senha, curso, universidade, foto_perfil_path) VALUES (?, ?, ?, ?, ?, ?)`;
  db.run(sql, [nome, email, senhaHash, curso, universidade, fotoPath], (err) => {
    if (err)
      return res
        .status(err.message.includes("UNIQUE") ? 409 : 500)
        .json({
          sucesso: false,
...
app.post("/api/cadastro/empresa", upload.single("logoEmpresa"), (req, res) => {
  ...
  bcrypt.hash(senha, 10, (err, hash) => {
    ...
    db.run(
      sql,
      [nome_empresa, email_corporativo, hash, ramo, logoPath],
      (err) => {
        if (err)
          return res
            .status(err.message.includes("UNIQUE") ? 409 : 500)
...
app.get("/api/alunos/:id", (req, res) => {
  ...
  const sqlAluno = `SELECT id, nome, curso, universidade, foto_perfil_path FROM alunos WHERE id = ?`;
...
app.get("/api/cidades", async (req, res) => {
  const { uf } = req.query;
  if (!uf) return res.status(400).json({ sucesso: false, mensagem: "UF obrigatória." });

  try {
    const resp = await axios.get(`https://brasilapi.com.br/api/ibge/municipios/v1/${uf}`);
...
// --- Servir Arquivos Estáticos e Rota Catch-all (ESSENCIAL QUE FIQUEM NO FINAL) ---
